#!/usr/bin/env python3
"""
Backfill task states from MongoDB to PostgreSQL.

This script should be run BEFORE enabling dual-write phase to ensure
existing task states are present in PostgreSQL.

Usage:
    python scripts/backfill_task_states.py [--dry-run] [--batch-size=1000]
...
async def backfill_states(
    ...
    """
    Backfill all task states from MongoDB to PostgreSQL.
    ...
    while True:
        try:
            states = await mongo_repo.list(limit=batch_size, page_number=page)
        ...
        for state in states:
            try:
                # Check if already exists in PostgreSQL by task_id + agent_id
                existing = await postgres_repo.get_by_task_and_agent(
                ...
                if dry_run:
                    logger.info(
                        f"[DRY RUN] Would migrate state {state.id} "
                        f"(task={state.task_id}, agent={state.agent_id})"
                    )
                    total_migrated += 1
                    continue

                # Create in PostgreSQL with the same ID
                await postgres_repo.create(state)
                total_migrated += 1

            except Exception as e:
                logger.error(
                    f"Error migrating state {state.id}: {e}",
                    extra={"task_id": state.task_id, "agent_id": state.agent_id},
                )
                total_errors += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill task states from MongoDB to PostgreSQL"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    migrated, skipped, errors = asyncio.run(
        backfill_states(dry_run=args.dry_run, batch_size=args.batch_size)
    )
