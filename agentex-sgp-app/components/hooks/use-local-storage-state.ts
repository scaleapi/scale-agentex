...
const SUPPORTS_QUEUE_MICROTASK =
  typeof window !== 'undefined' && 'queueMicrotask' in window;
...
const localStorageWrapper = createStorage(() =>
...
function defaultOrInitializer<S>(
  defaultValueOrInitializeFn:
    | S
    | ((value?: unknown, rawStorageValue?: unknown) => S),
  value?: unknown,
  rawValue?: unknown
): S {
        ...
        setValue((p) => {
          ...
          scheduleMicroTask(() => {
            localStorageWrapper.setItem(
              key,
              stringifyForStorage(newlyComputedValue)
            );
          });
