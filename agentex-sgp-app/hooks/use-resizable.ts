...
interface UseResizableOptions {
  ...
  /**
   * Whether to invert the drag direction (for right-side sidebars).
   * When true, dragging left increases width, dragging right decreases width.
   */
...
const useResizable = ({
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      // Invert deltaX for right-side sidebars (drag left = increase width)
      const adjustedDeltaX = invertDragDirection ? -deltaX : deltaX;
      const newWidth = Math.max(
        minWidth,
        Math.min(maxWidth, startWidthRef.current + adjustedDeltaX)
      );
    ...
    [ref, minWidth, maxWidth, invertDragDirection]
  );
  ...
  const handleMouseUp = useCallback(() => {
    ...
    onResizeEnd?.(newSize);
  }, [onResizeEnd, ref, initialSize, handleMouseMove]);
